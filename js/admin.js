...
export async function openVideoAnalytics(videoId) {
    ...
    try {
        ...
        if (state.activeCourseContext && state.activeCourseContext.id) {
            ...
            // Fetch users who have this course in their courseIds array
            let userQ = query(collection(db, "users"), where("courseIds", "array-contains", state.activeCourseContext.id));

            if (state.activeCourseContext.subcode) {
                userQ = query(collection(db, "users"), where("courseIds", "array-contains", state.activeCourseContext.id), where("subcourseCode", "==", state.activeCourseContext.subcode));
            }
