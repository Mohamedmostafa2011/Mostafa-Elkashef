...
window.switchToCourseSelection = () => {
    const courseIds = state.currentUserData.courseIds || (state.currentUserData.courseId ? [state.currentUserData.courseId] : []);
    state.activeCourseContext = null;
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        if (snap.exists()) {
            ...
            state.activeCourseContext = null; // FORCE RESET: Students always land on Course Hub first
